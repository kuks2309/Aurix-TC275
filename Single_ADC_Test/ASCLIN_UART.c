...
uint8 g_rxRcvBuffer[MAX_RCV_BUFFER_SIZE] = {0};
...
void init_ASCLIN_UART(void)
{
    ...
    ascConfig.loopBack = FALSE;  /* 명시적으로 비활성화 */
...
{
    /* Receive data via RX  */
    Ifx_SizeT count = (Ifx_SizeT)cnt;
    IfxAsclin_Asc_read(&g_ascHandle, g_rxData, &count, TIME_INFINITE);
}

/* Command decoding function */
int decode_command(void)
{
    return decode_adc_command();
}
...
int decode_adc_command(void)
{
    adc_group = 0;
    adc_channel = 0;
    for (int i = MAX_RCV_BUFFER_SIZE - 5; i >= 0; i--)
    {
        /* ADC 명령어 프로토콜: #A[group][channel]* */
        if ((g_rxRcvBuffer[i + 0] == '#') && (g_rxRcvBuffer[i + 1] == 'A') && (g_rxRcvBuffer[i + 4] == '*'))
        {
